...
Runs the analysis functions in objdetect.data.eda over the downloaded COCO
val2017 annotations, saves plots into reports/figures/, and writes a JSON
summary the EDA report quotes. Keeping this in a script (not only a notebook)
means the analysis is reproducible with one command and is covered by tests.

Usage:  uv run python scripts/run_eda.py
"""
...
def main() -> int:
    ...
    plt.ylabel("Class")
    plt.tight_layout()
    plt.savefig(config.FIGURES_DIR / "eda_class_distribution.png", dpi=120)
    plt.close()
    ...
    plt.tight_layout()
    plt.savefig(config.FIGURES_DIR / "eda_boxes_per_image.png", dpi=120)
    plt.close()
    ...
    plt.tight_layout()
    plt.savefig(config.FIGURES_DIR / "eda_relative_area.png", dpi=120)
    plt.close()
    ...
    plt.tight_layout()
    plt.savefig(config.FIGURES_DIR / "eda_cooccurrence.png", dpi=120)
    plt.close()
